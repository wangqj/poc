...
func (s *MongodbService) CreateOrUpdate(kv *KV) (*KV, error) {
	if kv.Domain == "" {
...
func (s *MongodbService) update(ctx context.Context, collection *mongo.Collection, kv *KV) error {
...
func (s *MongodbService) AddHistory(kv *KV) error {
...
func (s *MongodbService) getLatest(id primitive.ObjectID) (*KVHistory, error) {
	collection := s.c.Database(DB).Collection(CollectionRevision)
	ctx, _ := context.WithTimeout(context.Background(), DefaultTimeout)
